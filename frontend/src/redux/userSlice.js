  ...
  async (_, { rejectWithValue }) => {
    try {
      const res = await axios.get(
        "https://socialfooddelivery-2.onrender.com/api/v1/user/followings",
        {
  ...
  async (targetUserId, { rejectWithValue }) => {
    try {
      const res = await axios.post(
        `https://socialfooddelivery-2.onrender.com/api/v1/user/followorunfollow/${targetUserId}`,
        {},
        { withCredentials: true }
      );
...
export const getUserStats = createAsyncThunk(
  ...
  async (userId, { rejectWithValue }) => {
    try {
      const res = await axios.get(
        `https://socialfooddelivery-2.onrender.com/api/v1/user/stats/${userId}`,
        { withCredentials: true }
      );
      return res.data;
    } catch (err) {
...
export const { setFollowing, clearMessage, clearError, setLastAction } =
  userSlice.actions;
